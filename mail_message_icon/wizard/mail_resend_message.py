...
class MailResendMessage(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if message_id and self._context.get('show_all_mails'):
            mail_message_id = self.env['mail.message'].browse(message_id)
            notification_ids = mail_message_id.notification_ids.filtered(lambda notif: notif.notification_type == 'email' and notif.notification_status not in ('exception', 'bounce'))
            ...
            rec['notification_ids'] = rec.get("notification_ids", []) + [Command.set(notification_ids.ids)]
            # rec['mail_message_id'] = mail_message_id.id
            rec['partner_ids'] = rec.get("partner_ids", []) + partner_ids
        return rec
